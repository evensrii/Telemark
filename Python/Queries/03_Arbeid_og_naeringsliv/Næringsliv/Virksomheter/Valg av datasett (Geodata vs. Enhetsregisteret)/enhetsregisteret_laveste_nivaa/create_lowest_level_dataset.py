...
Additionally, this script includes "orphaned underenheter" - underenheter in Telemark
whose Level 1 parent organization is located outside Telemark region.
These are included as Level 3 entities since they have no parent chain in the dataset.
"""
...
# ===== ADD ORPHANED UNDERENHETER =====
print("\n" + "="*80)
print("ADDING ORPHANED UNDERENHETER (parent outside Telemark)")
print("="*80)

# Identify underenheter whose parent is NOT in Telemark
# These are underenheter whose 'Overordnet enhet' is not in the hovedenheter dataset
all_org_numbers_in_hoved = set(df_hoved['Org. nr.'].values)
orphaned_underenheter = df_under[
    (df_under['Overordnet enhet'] != '') & 
    (~df_under['Overordnet enhet'].isin(all_org_numbers_in_hoved))
].copy()

print(f"Found {len(orphaned_underenheter):,} orphaned underenheter (parent outside Telemark)")

# Add these as Level 3 entities
orphaned_count = 0
orphaned_with_employees = 0
orphaned_total_employees = 0

for _, entity in orphaned_underenheter.iterrows():
    entity_dict = entity.to_dict()
    entity_dict['Nivå enhetsregisteret'] = 3
    lowest_level_data.append(entity_dict)
    orphaned_count += 1
    
    # Track statistics
    employees = entity.get('Antall ansatte', 0)
    if pd.notna(employees) and employees > 0:
        orphaned_with_employees += 1
        orphaned_total_employees += employees

print(f"  - Total orphaned underenheter added: {orphaned_count:,}")
print(f"  - With employees: {orphaned_with_employees:,}")
print(f"  - Total employees: {orphaned_total_employees:,.0f}")

# Log orphaned decisions
for _, entity in orphaned_underenheter.iterrows():
    decision_log.append({
        'Org. nr.': entity['Org. nr.'],
        'Navn': entity['Navn'],
        'Level Used': 3,
        'Has Level 2': False,
        'Has Level 3': False,
        'Pct Diff 1v2': None,
        'Pct Diff 1v3': None,
        'Decision': f"Orphaned underenhet (parent {entity['Overordnet enhet']} outside Telemark) -> Use level 3"
    })
