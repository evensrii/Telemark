"""
Fylkessammenligning Query Script: Grunnskolepoeng etter foreldrenes utdanningsnivå.txt
==================================================================================

Auto-generated county-comparison variant of the FHI query
"Grunnskolepoeng etter foreldrenes utdanningsnivå.txt" (see FHI/scripts/ for the production, per-kommune version).
...
query_file = os.path.join(
    ...
    "fylkessammenligning",
    "queries",
    "Grunnskole", "Grunnskolepoeng etter foreldrenes utdanningsnivå.txt"
)

# Output configuration (local only - not pushed to GitHub)
output_filename = "grunnskolepoeng_etter_foreldrenes_utdanningsnivaa.csv"
...
# %%
print(f"\n{'=' * 70}")
print(f"Fylkessammenligning Query: Grunnskolepoeng etter foreldrenes utdanningsnivå.txt")
print(f"{'=' * 70}\n")
...
# FHI labels this table's Måltall "andel (prosent, standardisert)", which
# triggered the standard /100 scaling above - but the values are actual
# grunnskolepoeng averages (typically ~30-50), not a percentage. Undo the
# incorrect scaling; the column stays named "Andel" for consistency with the
# rest of the pipeline, but its values are now the real point average.
