"""
FHI Query Script: barnevern.txt
===============================

Auto-generated script for processing FHI query data.
Query file: Oppvekst og levekår/Barnevern/barnevern.txt
...
query_file = os.path.join(
    ...
    "FHI", 
    "queries",
    "Oppvekst og levekår", "Barnevern", "barnevern.txt"
)

# Output configuration
output_filename = "barnevern.csv"
github_folder = "Data/08_Folkehelse og levekår/Oppvekst og levekår/Barnevern"
...
# %%
print(f"\n{'=' * 70}")
print(f"FHI Query: barnevern.txt")
print(f"{'=' * 70}\n")
...
# Capitalize first letter in Kjønn
df['Kjønn'] = df['Kjønn'].str.capitalize()

# Replace "_" with "-" in ALDER and rename to Alder
df['ALDER'] = df['ALDER'].str.replace('_', '-')
df = df.rename(columns={'ALDER': 'Alder'})

# Replace ":" with empty string
df['value'] = df['value'].replace(':', '')

# Rename value to Andel and round to 1 decimal
df['value'] = pd.to_numeric(df['value'], errors='coerce') / 100
df['value'] = df['value'].round(3)
df = df.rename(columns={'value': 'Andel'})

# Remove Måltall column
