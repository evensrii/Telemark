...
def discover_available_years():
    ...
    Validates the latest year(s) against the export API, since WFS may
    list layers before the data is actually available for download.
    ...
    # Validate latest year(s) - WFS may list layers before export data is ready
    while years:
        latest = years[-1]
        alias = f"{DATASET_ALIAS_PREFIX}{latest}"
        payload = {"dataset": alias, "format": "Csv", "attributes": ["SSBID0250M"],
                   "srid": str(EXPORT_SRID), "language": EXPORT_LANGUAGE}
        try:
            resp = requests.post(EXPORT_URL, json=payload)
            if resp.status_code == 404:
                print(f"  Year {latest} listed in WFS but not available for export (404), removing.")
                years.pop()
            else:
                break  # Year exists (200, or other non-404 status)
        except Exception:
            break  # Network issue, keep remaining years
