...
MASTER_LOG_FILE = os.path.join(LOG_DIR, "00_master_run_weekly.log")
EMAIL_LOG_FILE = os.path.join(LOG_DIR, "00_email_weekly.log")
CONDA_ENV = "analyse"
...
def send_email():
    ...
    with open(EMAIL_LOG_FILE, "w", encoding="utf-8") as email_log:
        ...
        # Quote the script path to handle spaces and add --weekly flag
        command = f'cmd.exe /c "conda activate {CONDA_ENV} && python \"{email_script_path}\" --weekly"'
